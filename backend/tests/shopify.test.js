/**
 * REVIXA BACKEND — SHOPIFY OAUTH & WEBHOOKS TEST SUITE (JEST + SUPERTEST)
...
import { ShopifyRepository } from '../src/repositories/ShopifyRepository.js';
import { encryptToken, decryptToken } from '../src/utils/crypto.js';

describe('Phase 2.4 — Shopify OAuth & Webhook Test Suite', () => {
  ...
  describe('AES-256-GCM Encryption Round-Trip', () => {
    it('should save an encrypted connection, retrieve it, decrypt it, and assert it matches the original plaintext token', async () => {
      ...
      await ShopifyRepository.saveConnection(orgId, validShop, encrypted);
      const retrievedConnection = await ShopifyRepository.getConnection(orgId);
  ...
  describe('GET /auth/shopify/install', () => {
    ...
    it('should consume and invalidate state nonce after first use to prevent replay attacks', async () => {
      ...
      await ShopifyRepository.saveOAuthState(nonceState, 'org_replay', Date.now() + 600000);

      // First consumption: should return entry
      const firstConsume = await ShopifyRepository.verifyAndConsumeOAuthState(nonceState);
      ...
      const secondConsume = await ShopifyRepository.verifyAndConsumeOAuthState(nonceState);
  ...
  describe('GET /auth/shopify/callback', () => {
    ...
    it('should reject callback with 401 Unauthorized if HMAC signature is invalid', async () => {
      ...
      await ShopifyRepository.saveOAuthState(stateNonce, 'org_test_1', Date.now() + 600000);
    ...
    it('should successfully complete OAuth callback, store token ENCRYPTED, and link organization', async () => {
      ...
      await ShopifyRepository.saveOAuthState(stateNonce, orgId, Date.now() + 600000);
  ...
  // 4. Webhook testing (app/uninstalled, orders/create, orders/updated)
  describe('Shopify Webhook Handlers', () => {
    describe('POST /webhooks/shopify/app/uninstalled', () => {
      ...
      it('should process app/uninstalled webhook, mark connection inactive, and preserve data', async () => {
        ...
        const connection = await ShopifyRepository.getConnectionByShop(validShop);
        expect(connection).not.toBeNull();
    ...
    describe('POST /webhooks/shopify/orders/create', () => {
      it('should reject orders/create webhook with 401 Unauthorized if HMAC signature is invalid', async () => {
        const res = await request(app)
          .post('/webhooks/shopify/orders/create')
          .set('X-Shopify-Hmac-Sha256', 'invalid_order_hmac_123')
          .set('X-Shopify-Shop-Domain', validShop)
          .send({ order_number: 1088, total_price: '295.00' });
    ...
    describe('POST /webhooks/shopify/orders/updated', () => {
      it('should reject orders/updated webhook with 401 Unauthorized if HMAC signature is invalid', async () => {
        const res = await request(app)
          .post('/webhooks/shopify/orders/updated')
          .set('X-Shopify-Hmac-Sha256', 'invalid_order_hmac_456')
          .set('X-Shopify-Shop-Domain', validShop)
          .send({ order_number: 1088, financial_status: 'paid' });
      ...
      it('should process orders/updated webhook correctly when HMAC signature is valid', async () => {
        const payloadObj = { order_number: 1088, financial_status: 'paid' };
        const rawBody = JSON.stringify(payloadObj);
        const validWebhookHmac = crypto
          .createHmac('sha256', apiSecret)
          .update(rawBody, 'utf8')
        ...
        expect(res.status).toBe(200);
        expect(res.body.status).toBe('success');
        expect(res.body.data.topic).toBe('orders/updated');
        expect(res.body.data.processed).toBe(true);
        expect(res.body.data.orderNumber).toBe(1088);
      });
    });
